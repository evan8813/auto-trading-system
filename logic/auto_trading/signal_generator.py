...
進出場規格：
  做多進場：收盤 > 前日 High_N(50)  且  MA_fast > MA_slow  且  量 > Vol_MA20 × vol_mult
  做多出場：收盤 < trail_high - atr_mult × ATR（ATR 追蹤停損，從第一天開始）
  做空進場：收盤 < 前日 Low_N(50)   且  MA_fast < MA_slow  且  量 > Vol_MA20 × vol_mult
...
class SignalGenerator:
    ...
    def long_entry(row: pd.Series, prev_row: pd.Series, vol_mult: float = 1.5) -> bool:
        ...
        做多進場條件：
          ...
          2. MA_fast > MA_slow（多頭趨勢確認）
          3. 當日成交量 > 20 日均量 × vol_mult（突破量能確認）
        """
        ...
        return breakout and ma_trending and vol_surge
    ...
    def short_entry(row: pd.Series, prev_row: pd.Series, vol_mult: float = 1.5) -> bool:
        ...
        做空進場條件：
          ...
          2. MA_fast < MA_slow（空頭趨勢確認）
          3. 當日成交量 > 20 日均量 × vol_mult（跌破量能確認）
        """
        ...
        ma_trending = row["MA_fast"] < row["MA_slow"]
        vol_surge   = row["Volume"]  > row["Vol_MA20"] * vol_mult
        return breakdown and ma_trending and vol_surge
