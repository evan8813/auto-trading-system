...
class TestBacktesterRun:
    ...
    def test_warmup_skipped_all_indicators_valid_on_start(self):
        """
        equity_curve 的第一個日期，至少一支股票的所有指標欄均非 NaN。
        確認暖機期是等所有指標都齊了才開始，而不只是 ATR。
        """
        from indicators import Indicators
        _base_cols = {"Open", "High", "Low", "Close",
                      "Volume", "Amount", "Change", "Transactions"}
        data_with_ind = {
            t: Indicators.add_all(df, self.cfg)
            for t, df in self.data.items()
        }
        indicator_cols = [
            c for c in next(iter(data_with_ind.values())).columns
        ...
        # 至少一支股票在起始日的所有指標欄均有值
        has_all_valid = any(
            first_date in df.index and
            all(not pd.isna(df.loc[first_date, c])
                for c in indicator_cols if c in df.columns)
            for df in data_with_ind.values()
        )
        assert has_all_valid, (
            f"起始日 {first_date} 沒有任何股票的指標欄全部有值，"
            f"暖機期未正確等到所有指標就緒（需要：{indicator_cols}）"
        )

    def test_equity_curve_start_date_not_before_warmup_complete(self):
        """
        equity_curve 起始日不應等於 backtest_start，
        確認暖機期（最長指標週期）已被跳過。
        """
        result     = self.bt.run(self.data)
        first_date = result["equity_curve"].index[0]
        ...
        assert first_date > start, (
            f"equity_curve 起始日 {first_date} 等於 backtest_start，"
            f"暖機期可能未被跳過"
        )
