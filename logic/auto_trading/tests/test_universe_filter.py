...
測試項目：
  1. 同時符合流動性與近52週高點 → 納入候選
  2. 流動性不足 → 排除
  3. 距52週高點過遠 → 排除
  4. 兩個條件都不符合 → 排除
  5. 指定日期不在 DataFrame → 跳過
  6. Avg_Amount_20 為 NaN → 排除
  7. High_52W 為 NaN 或 0 → 排除
  8. 多支股票篩選結果正確
"""
...
def make_filter(min_avg_amount: float = 5_000_000) -> UniverseFilter:
    cfg = TradingConfig(min_avg_amount=min_avg_amount)
...
class TestUniverseFilterPassConditions:
    ...
    def test_passes_both_conditions(self):
        ...
        # close=95 >= 100×0.90=90 ✓；avg_amount=10M >= 5M ✓
...
class TestUniverseFilterFailConditions:
    ...
    def test_fails_both_conditions(self):
        uf  = make_filter(min_avg_amount=5_000_000)
        df  = make_df_with_row("2023-06-01", close=70.0, high_52w=100.0, avg_amount_20=1_000_000)
        result = uf.filter({"2330": df}, pd.Timestamp("2023-06-01"))
        assert "2330" not in result
...
class TestUniverseFilterMultipleStocks:
    ...
    def test_filters_correct_subset(self):
        ...
        data = {"2330": df_pass, "2317": df_low_liq, "2454": df_far_high}
