...
篩選條件（依 CLAUDE.md 規格）：
  ...
  3. 收盤 × 1000 <= max_trade_cost（單倉買得起 1 張）
"""
...
def make_filter(
    ...
    max_trade_cost: float = 36_000.0,
) -> UniverseFilter:
    cfg = TradingConfig(min_avg_amount=min_avg_amount, max_trade_cost=max_trade_cost)
...
class TestUniverseFilterPassConditions:
    ...
    def test_passes_all_conditions(self):
        ...
        uf     = make_filter()
        df     = make_df(high=110.0, low=96.0, close=30.0, avg_amount=10_000_000,
                         prev_high_52w=105.0, prev_low_52w=95.0)
        result = uf.filter({"2330": df}, DATE_TODAY)
    ...
    def test_passes_at_exact_max_price(self):
        """股價剛好等於單倉上限（max_trade_cost÷1000，邊界值應通過）"""
        # max_trade_cost=36,000 → max_price=36 元/股；close=36 剛好等於上限
        uf     = make_filter(max_trade_cost=36_000.0)
...
class TestUniverseFilterFailConditions:
    ...
    def test_fails_too_expensive(self):
        """股價超出單倉上限"""
        # max_trade_cost=36,000 → max_price=36；close=37 > 36
        uf     = make_filter(max_trade_cost=36_000.0)
...
class TestUniverseFilterMultipleStocks:
    ...
    def test_filters_correct_subset(self):
        ...
        # 2330：三條件全過（高點突破、均量足、股價可負擔）
        df_pass   = make_df(high=110.0, low=96.0, close=30.0, avg_amount=10_000_000,
                            prev_high_52w=105.0, prev_low_52w=95.0)
        # 2317：均量不足
